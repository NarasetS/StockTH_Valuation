...
st.caption(
    "This version fixes the broken control-flow in v3 (iv/pv_sum NameErrors) and keeps reverse DCF "
    "as fixed-step gradient descent (not Newton-Raphson)."
)
...
def solve_implied_growth_fixed_step(
    ...
    # Fixed-step update: gr <- gr - step * f(gr)
...
with st.sidebar:
    ...
    st.subheader("Reverse DCF solver controls")
    ...
    gd_max_iter = st.number_input("Max iterations", value=200000, min_value=1000, step=1000)
...
with colB:
    ...
    if np.isnan(current_price) or current_price <= 0:
        ...
        dr_grid = np.round(np.linspace(dr_start, 0.30, 30), 4)
        ...
        # st.line_chart(df_curve.set_index("Discount rate"))
        chart = (
            alt.Chart(df_curve)
            .mark_line()
            .encode(
                x=alt.X(
                    "Discount rate:Q",
                    title="Discount rate (dr)",
                    axis=alt.Axis(format=".0%")
                ),
                y=alt.Y(
                    "Implied growth (gr):Q",
                    title="Implied FCF growth rate (gr)",
                    axis=alt.Axis(format=".0%")
                ),
                tooltip=[
        ...
        # Sensitivity table (IV/share across dr x gr)
